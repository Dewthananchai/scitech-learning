...
"ข้อใดไม่ใช่สิ่งมีชีวิต?","แมว","ต้นไม้","ก้อนหิน","ปลา","C","ก้อนหินไม่ใช่สิ่งมีชีวิต",1
"ข้อใดจัดเป็นสิ่งมีชีวิตได้ถูกต้อง?","น้ำ","ดิน","ผีเสื้อ","หิน","C","ผีเสื้อเป็นสัตว์ที่มีชีวิต",1`;

export default function EditLesson() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { lessons, questions, updateLesson, addQuestionsBatch, deleteQuestion, subjects } = useAppStore();
  ...
  const [form, setForm] = useState({
    ...
    description: '', coverImage: lesson?.cover_image || null as string | null,
  ...
  // Load existing questions for this lesson
  const existingQuestions = useMemo(() => {
    return questions.filter(q => q.lesson_id === Number(id));
  }, [questions, id]);
  ...
  useEffect(() => {
    ...
    const objMatch = lesson.content.match(/## เป้าหมายการเรียนรู้\n([\s\S]*?)\n\n---\n\n/);
    if (objMatch) {
      objectives = objMatch[1].trim();
      content = lesson.content.replace(/## เป้าหมายการเรียนรู้\n[\s\S]*?\n\n---\n\n/, '');
    }
    setForm({
      title: lesson.title,
      grade: subject ? String(subject.grade_level) : '',
      subject: 'วิทยาศาสตร์',
      topic: subject?.description || '',
      unit: String(lesson.subject_unit_id),
      description: lesson.summary || '',
      coverImage: lesson?.cover_image || null,
      mediaType: (lesson.media_type as MediaType) || 'none',
      mediaUrl: lesson.media_url || '',
      pdfFile: null,
      mediaItems: lesson.media_items || [],
      content: content,
  ...
  const handleDeleteNewQuestion = (idx: number) => {
    setNewQuestions(prev => prev.filter((_, i) => i !== idx));
  };

  const handleDeleteExistingQuestion = (qId: number) => {
    if (confirm('ต้องการลบข้อสอบนี้?')) {
      deleteQuestion(qId);
    }
  };
      ...
      try {
        ...
        headers.forEach((h, i) => {
          const map: Record<string, string> = {
            ...
            'คำอธิบาย': 'explanation', 'ระดับความยาก': 'difficulty',
          };
        ...
        const imported: Omit<Question, 'id'>[] = [];
        for (let i = 1; i < rows.length; i++) {
          ...
          imported.push({
            ...
            difficulty: parseInt(get('difficulty')) || 1, is_active: true,
          });
        }
        if (imported.length > 0) {
          setNewQuestions(prev => [...prev, ...imported]);
  ...
  const handleSave = (publish: boolean) => {
    ...
    updateLesson(Number(id), {
      subject_unit_id: Number(form.unit), title: form.title,
      content: contentWithObjectives, summary: form.description || form.title,
      ...
      difficulty: form.difficulty, estimated_minutes: form.estimatedMinutes,
      is_published: publish,
    });

    // Save new questions
    if (newQuestions.length > 0) {
      addQuestionsBatch(newQuestions);
    }

    setTimeout(() => {
      setSaving(false);
      alert(`✅ บันทึกสำเร็จ! (+ ${newQuestions.length} ข้อสอบใหม่)`);
  ...
  if (!lesson) {
    return (
      <div className="min-h-screen bg-slate-50">
        <MobileHeader title="แก้ไขบทเรียน" />
        <AdminSidebar />
        <main className="md:ml-64 p-4 md:p-6 pt-16 md:pt-6">
          <div className="text-center py-20">
            ...
            <Link to="/admin/lessons" className="text-blue-600 hover:underline">← กลับหน้าคลังบทเรียน</Link>
          </div>
        </main>
      </div>
    );
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input type="text" value={form.title} onChange={e => updateField('title', e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none" />
              </div>
              <div>
              ...
              <div>
                <label className="text-sm font-medium block mb-1">กลุ่มสาระการเรียนรู้</label>
                <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" disabled><option>วิทยาศาสตร์</option></select>
              </div>
              <div>
                <label className="text-sm font-medium block mb-1">สาระการเรียนรู้</label>
                <select value={form.topic} onChange={e => updateField('topic', e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none">
                  ...
                  <option value="bio">วิทยาศาสตร์ชีวภาพ</option><option value="phys">วิทยาศาสตร์กายภาพ</option>
                  <option value="earth">โลกและอวกาศ</option><option value="tech">เทคโนโลยี</option>
                </select>
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea value={form.description} onChange={e => updateField('description', e.target.value)} rows={3} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none"></textarea>
              </div>
              <div>
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea value={form.objectives} onChange={e => updateField('objectives', e.target.value)} rows={5} placeholder={"นักเรียนสามารถ...\n1. \n2."} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none"></textarea>
              </div>
              <div>
                <label className="text-sm font-medium block mb-1">เกณฑ์ประเมิน</label>
                <textarea value={form.criteria} onChange={e => updateField('criteria', e.target.value)} rows={3} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none"></textarea>
              </div>
            </div>
          </div>
        );
      case 3:
        return (
          <div>
            ...
            <div className="mb-6">
              <label className="text-sm font-medium block mb-1">เนื้อหาบทเรียน * (Markdown)</label>
              <textarea value={form.content} onChange={e => updateField('content', e.target.value)} rows={12} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none font-mono"></textarea>
            </div>
              ...
              {form.mediaType === 'pdf' && (
                <div className="bg-red-50 rounded-xl p-4 mt-4">
                  ...
                  <div className="border-2 border-dashed border-red-200 rounded-xl p-6 text-center hover:border-red-400 transition-colors cursor-pointer">
                    ...
                    <input type="file" accept=".pdf" multiple className="hidden" id="pdf-edit" onChange={e => {
                    ...
                    <label htmlFor="pdf-edit" className="mt-2 inline-block bg-red-500 text-white px-4 py-2 rounded-lg text-sm cursor-pointer hover:bg-red-600">เลือกไฟล์ (เลือกได้หลายไฟล์)</label>
                  </div>
                </div>
              )}
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => downloadSampleCSV()} className="flex items-center gap-2 px-3 py-2 bg-slate-100 text-slate-600 rounded-lg text-sm hover:bg-slate-200">⬇ ตัวอย่าง CSV</button>
                <button onClick={() => setShowQuizForm(!showQuizForm)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="text-sm font-medium block mb-1">ระดับยาก</label>
                  ...
                  <div>
                    ...
                    <textarea value={quizForm.explanation} onChange={e => setQuizForm(p => ({ ...p, explanation: e.target.value }))} placeholder="อธิบายเหตุผล..." className="w-full border border-blue-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none" rows={2} />
                  </div>
            ...
            {existingQuestions.length > 0 && (
              <div className="mb-4">
                <h4 className="text-sm font-medium text-gray-600 mb-2">📋 ข้อสอบเดิม ({existingQuestions.length} ข้อ)</h4>
                <div className="space-y-2">
                  {existingQuestions.map((q, idx) => (
                    <div key={q.id} className="bg-white border border-slate-200 rounded-xl p-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-xs font-bold">{idx + 1}</span>
                        <span className="text-sm text-gray-800 truncate max-w-[300px]">{q.question_text}</span>
                        <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded text-xs font-bold">เฉลย: {q.correct_answer}</span>
                      </div>
                      <button onClick={() => handleDeleteExistingQuestion(q.id)} className="text-red-500 hover:text-red-700 text-sm">🗑️ ลบ</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {newQuestions.length > 0 && (
              <div className="mb-4">
                <h4 className="text-sm font-medium text-blue-600 mb-2">🆕 ข้อสอบใหม่ ({newQuestions.length} ข้อ)</h4>
                <div className="space-y-2">
                  {newQuestions.map((q, idx) => (
                    <div key={idx} className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="bg-blue-100 text-blue-600 px-2 py-0.5 rounded text-xs font-bold">{existingQuestions.length + idx + 1}</span>
                        <span className="text-sm text-gray-800 truncate max-w-[300px]">{q.question_text}</span>
                        <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded text-xs font-bold">เฉลย: {q.correct_answer}</span>
                      </div>
                      <button onClick={() => handleDeleteNewQuestion(idx)} className="text-red-500 hover:text-red-700 text-sm">🗑️ ลบ</button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${!form.isPublished ? 'border-blue-300 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'}`}>
                ...
                <div><p className="font-medium">ฉบับร่าง</p><p className="text-sm text-slate-500">บทเรียนจะยังไม่แสดงให้นักเรียนเห็น</p></div>
              </label>
              <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${form.isPublished ? 'border-green-300 bg-green-50' : 'border-slate-200 hover:bg-slate-50'}`}>
                ...
                <div><p className="font-medium">เผยแพร่</p><p className="text-sm text-slate-500">บทเรียนจะแสดงให้นักเรียนเห็นทันที</p></div>
              </label>
            </div>
            <div className="mt-6 bg-slate-50 rounded-xl p-4">
              ...
              <div className="text-sm text-slate-600 space-y-1">
                ...
                <p>• ข้อสอบ: <strong>{existingQuestions.length + newQuestions.length} ข้อ</strong></p>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link
              to={`/lesson/${id}`}
              target="_blank"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/80 hover:bg-white border border-emerald-100 text-xs font-bold text-emerald-700 shadow-sm transition-all"
            >
              <span>📖 ดูหน้าเรียนจริง</span>
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-emerald-100/80 text-xs md:text-sm mt-1">
                  ปรับปรุงเนื้อหา สื่อการสอน และคลังแบบทดสอบให้ทันสมัย
                </p>
              </div>

              {/* Step indicator in topbar */}
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-2.5 border border-white/20">
                <div className="text-[11px] font-bold text-emerald-100 px-1 mb-1.5">ความคืบหน้าการแก้ไข</div>
